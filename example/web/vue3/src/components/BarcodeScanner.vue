<template>
  <div v-once class="component-barcode-scanner" ref="root">
  ...
  setup(props, context) {
    ...
    onMounted(async () => {
      try {
        ...
        await scanner.value.setUIElement(root.value);
        scanner.value.onFrameRead = (results) => {
          if (results.length) {
            console.log(results);
          }
        };
        scanner.value.onUnduplicatedRead = (txt, result) => {
          context.emit(
            "appendMessage",
            result.barcodeFormatString + ": " + txt
          );
        };
        await scanner.value.open();
      } catch (ex) {
        context.emit("appendMessage", ex.message);
        console.error(ex);
      }
    });
